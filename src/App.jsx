...
function App() {
	...
	return (
		<div className='todo-box'>
			...
			<form className={text.length < 5 ? 'error' : ''}>
				<input type="text" placeholder='Text....' value={text} onChange={changeText} />
				<input type="button" value="Click" onClick={addText} disabled={text.length < 5} />
			</form>
			<ul>
				{local.map((element, index) => (
					<li key={index}>
						<label htmlFor={index}>
							<input type="checkbox" name="" id={index} onClick={checkList(index)} />
							{element.text}
